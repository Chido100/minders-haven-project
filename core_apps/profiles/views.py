...
class ProfileListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self) -> List[Profile]:
        return (
            ...
            .filter(occupation=Profile.Occupation.PARENT)
        )
...
class NonParentProfileListAPIView(generics.ListAPIView):
    serializer_class = ProfileSerializer
    renderer_classes = [GenericJSONRenderer]
    pagination_class = StandardResultsSetPagination
    object_label = "non_parent_profiles"
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    search_fields = ["user__username", "user__first_name", "user__last_name"]
    filterset_fields = ["gender", "city"]
    ...
    def get_queryset(self) -> List[Profile]:
        return (
            Profile.objects.exclude(user__is_staff=True)
            .exclude(user__is_superuser=True)
            .exclude(occupation=Profile.Occupation.PARENT)
        )
